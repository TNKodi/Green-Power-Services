...
class ThingsBoardClient:
    ...
    async def get_entity_name(self, entity_type: str, entity_id: str) -> str:
        """
        Get display name for a ThingsBoard entity.

        Args:
            entity_type: Entity type (ASSET or DEVICE)
            entity_id: Entity ID

        Returns:
            Entity name if available, otherwise entity_id
        """
        normalized_type = (entity_type or "").upper()
        cache_key = f"{normalized_type}:{entity_id}"
        if cache_key in self._entity_name_cache:
            return self._entity_name_cache[cache_key]
        ...
        url = f"{self.base_url}/api/{endpoint}/{entity_id}"
        headers = await self._get_headers()

        try:
            response = await self._client.get(url, headers=headers)
            response.raise_for_status()
            name = response.json().get("name") or entity_id
            self._entity_name_cache[cache_key] = name
            return name
        except httpx.HTTPError as e:
            logger.warning(f"Unable to fetch {normalized_type} name for {entity_id}: {e}")
            return entity_id
    ...
    async def get_device_relations(self, asset_id: str) -> List[str]:
        """
        Get all child devices related to a parent asset.

        Args:
            asset_id: Parent asset ID

        Returns:
            List of child device IDs
        """
        url = f"{self.base_url}/api/relations/info"
        params = {
            "fromId": asset_id,
        ...
        try:
            ...
            for rel in relations:
                if rel.get("to", {}).get("entityType") == "DEVICE" and rel.get("type") == "Contains":
                    device_id = rel["to"]["id"]
                    devices.append(device_id)

            logger.debug(f"Found {len(devices)} child devices for asset {asset_id}")
            return devices

        except httpx.HTTPError as e:
            logger.error(f"Failed to get device relations for asset {asset_id}: {e}")
            raise
    ...
    async def write_device_telemetry(
        ...
        """
        await self.write_entity_telemetry("ASSET", device_id, telemetry)

    async def write_entity_telemetry(
        self,
        entity_type: str,
        entity_id: str,
        ...
        """
        Write telemetry data to a ThingsBoard entity.

        Args:
            entity_type: ThingsBoard entity type (ASSET or DEVICE)
            entity_id: Entity ID
            telemetry: Single telemetry record or list of telemetry records
        """
        normalized_type = (entity_type or "ASSET").upper()
        if normalized_type not in {"ASSET", "DEVICE"}:
            raise ValueError(f"Unsupported entity_type: {entity_type}")

        url = f"{self.base_url}/api/plugins/telemetry/{normalized_type}/{entity_id}/timeseries/ANY"
        headers = await self._get_headers()
        ...
        try:
            ...
            logger.debug(f"Wrote telemetry to {normalized_type} {entity_id}")

        except httpx.HTTPError as e:
            logger.error(f"Failed to write telemetry to {normalized_type} {entity_id}: {e}")
            raise
    ...
    async def write_bulk_telemetry(
        ...
        # ThingsBoard accepts a list payload for timeseries write; send once per asset.
        await self.write_entity_telemetry("ASSET", device_id, telemetry_list)
        
        logger.info(f"Successfully wrote {len(telemetry_list)} records to {device_id} in one API call")

    async def write_bulk_telemetry_to_entity(
        self,
        entity_type: str,
        entity_id: str,
        telemetry_list: List[Dict[str, Any]]
        ...
        """
        Write multiple telemetry records to a ThingsBoard entity.
        ...
        """
        normalized_type = (entity_type or "ASSET").upper()
        logger.info(f"Writing {len(telemetry_list)} telemetry records to {normalized_type} {entity_id}")

        if not telemetry_list:
            logger.warning(f"No telemetry records to write for {normalized_type} {entity_id}")
            return

        await self.write_entity_telemetry(normalized_type, entity_id, telemetry_list)

        logger.info(
            f"Successfully wrote {len(telemetry_list)} records to {normalized_type} {entity_id} in one API call"
        )
